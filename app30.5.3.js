            ...
            try {
                tg.sendData(JSON.stringify({ order, total }))
                    .then(() => {
                        ...
                        // Ожидаем ответ от бота с QR-кодом
                        return new Promise((resolve, reject) => {
                            let qrCodeTimeout = setTimeout(() => {
                                reject(new Error('Timeout waiting for QR code'));
                            }, 30000); // 30 секунд на ожидание QR-кода
                    ...
                    .catch((error) => {
                        ...
                        tg.showAlert('Произошла ошибка при обработке заказа. Пожалуйста, попробуйте еще раз.');
                    });
            } catch (error) {
                clearTimeout(timeoutId);
...
// Добавляем обработку ошибок для tg.sendData
