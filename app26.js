...
let audioContext;
let audioBuffer = ''; // Изменено на строку для накопления base64 данных
let tg;
let socket;
        ...
        try {
            if (typeof event.data === 'string') {
                ...
                if (data.type === 'ai-response-delta' && data.formatted && data.formatted.audio) {
                    ...
                    audioBuffer += data.formatted.audio; // Накапливаем base64 строку
                    ...
                    if (audioBuffer.length > 0) {
                        ...
                        audioBuffer = ''; // Очистка буфера после обработки
                    }
                }
...
function processAudioData(base64Audio) {
    // 1. Base64 PCM16 → двоичная строка
    const binaryString = atob(base64Audio);

    // 2. Двоичная строка → ArrayBuffer
    const pcm16ArrayBuffer = new ArrayBuffer(binaryString.length);
    const view = new Uint8Array(pcm16ArrayBuffer);
    for (let i = 0; i < binaryString.length; i++) {
        view[i] = binaryString.charCodeAt(i);
    }

    // 3. Создание WAV заголовка
    const wavHeader = createWavHeader(24000, 16, 1, pcm16ArrayBuffer.byteLength);

    // 4. Объединение WAV заголовка и PCM16 данных
    ...
    // 5. Декодирование и воспроизведение
