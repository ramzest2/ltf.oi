...
function decodeAudioData(arrayBuffer) {
    ...
    // Преобразование декодированной строки обратно в ArrayBuffer
    const decodedBuffer = new ArrayBuffer(decodedString.length);
...
function playAudio(arrayBuffer) {
    ...
    try {
        ...
        // Шаг 6: Расшифровка полученного ArrayBuffer
        const decodedArrayBuffer = decodeAudioData(arrayBuffer);

        // Проверка наличия WAV-заголовка
        const dataView = new DataView(decodedArrayBuffer);
        ...
        audioContext.decodeAudioData(decodedArrayBuffer, 
            (audioBuffer) => {
