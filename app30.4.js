...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.Telegram && window.Telegram.WebApp) {
        ...
        // ИЗМЕНЕНО: Обновлена функция addToCart
        function addToCart(id, name, price, quantity = 1) {
            console.log(`Добавление в корзину: id=${id}, name=${name}, price=${price}, quantity=${quantity}`);
            
            // Убедимся, что для шаурмы всегда используется правильный id
        ...
        document.querySelectorAll('.btn').forEach(btn => {
            if (btn) {
                btn.addEventListener('click', function() {
                    ...
                    let price = parseInt(this.previousElementSibling.dataset.price);
                    addToCart(id, name, price);
                });
            }
        });

        // ДОБАВЛЕНО: Новая функция для удаления товара из корзины
        ...
        // ИЗМЕНЕНО: Обновлена функция updateCartDisplay
        ...
        // ИЗМЕНЕНО: Обновлена функция formatPrice
