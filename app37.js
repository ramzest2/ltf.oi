        ...
        try {
            ...
            if (typeof event.data === 'string') {
                ...
                if (data.type === 'ai-response-delta' && data.formatted && data.formatted.audio) {
                    console.log('Получена аудио дельта');
                    processAudioData(data.formatted.audio);
...
function processAudioData(audioData) {
    ...
    if (typeof audioData === 'string') {
        // Если данные пришли в виде Base64 строки
        const binaryString = atob(audioData);
        newData = new Uint8Array(binaryString.length);
        for (let i = 0; i < binaryString.length; i++) {
            newData[i] = binaryString.charCodeAt(i);
        }
    } else if (audioData instanceof ArrayBuffer) {
        // Если данные уже в формате ArrayBuffer
        newData = new Uint8Array(audioData);
    } else {
    ...
    if (audioBuffer === null) {
        ...
        const combinedBuffer = new Uint8Array(audioBuffer.length + newData.length);
        combinedBuffer.set(audioBuffer, 0);
        combinedBuffer.set(newData, audioBuffer.length);
        audioBuffer = combinedBuffer;
    }

    console.log('Текущая длина аудиобуфера:', audioBuffer.length);
}

function finalizeAndPlayAudio() {
    if (audioBuffer === null || audioBuffer.length === 0) {
        console.error('Нет данных для воспроизведения');
        return;
    }

    console.log('Финализация и воспроизведение аудио, длина данных:', audioBuffer.length);

    const wavHeader = createWavHeader(24000, 16, 1, audioBuffer.length);
    const finalArrayBuffer = new ArrayBuffer(wavHeader.byteLength + audioBuffer.length);
    new Uint8Array(finalArrayBuffer).set(new Uint8Array(wavHeader), 0);
    new Uint8Array(finalArrayBuffer).set(audioBuffer, wavHeader.byteLength);

    playAudio(finalArrayBuffer);
    audioBuffer = null; // Сбрасываем буфер после воспроизведения
}
