...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.Telegram && window.Telegram.WebApp) {
        ...
        function validateOrder(order) {
         ...
         async function placeOrder() {
            ...
            try {
                ...
                console.log('Отправка запроса на FastAPI:', orderData);
        
                // Отправляем запрос на FastAPI endpoint
                const response = await fetch('https://your-domain.com/api/create-payment', {
                    method: 'POST',
                    headers: {
                ...
                console.log('Получен ответ от сервера:', paymentData);
                ...
                if (paymentData.status === 'success') {
                    ...
                    currentOrderId = paymentData.order_id;
                ...
                if (retryCount < maxRetries) {
                    ...
                    tg.showAlert('Превышено количество попыток. Пожалуйста, попробуйте позже.');
                }
            }
        }
        ...
        async function checkPaymentStatus(orderId) {
            try {
                const response = await fetch(`https://your-domain.com/api/check-payment/${orderId}`);
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
                }
                ...
                switch(statusData.status) {
                    ...
                    case 'cancel':
                    case 'deny':
                        showPaymentError(statusData.status);
                        closeQRCode();
                        break;
                    default:
                        console.warn('Неизвестный статус:', statusData.status);
                }
            } catch (error) {
        ...
        // Обновленная функция отображения QR кода
        function displayQRCode(qrCodeUrl, expiryTime) {
            console.log('Отображение QR кода:', qrCodeUrl);
            
            const qrContainer = document.getElementById('qr-container');
            const qrImage = document.getElementById('qr-image');
            const overlay = document.getElementById('overlay');
            
            if (!qrContainer || !qrImage || !overlay) {
            ...
            qrImage.onerror = function() {
                ...
                showError('Не удалось загрузить QR код');
            };
        ...
        tg.onEvent('message', function(message) {
            try {
                ...
                if (data.status === 'success' && data.payment_data) {
                    displayQRCode(
                        data.payment_data.qr_url,
                        data.payment_data.expiry_time
                    );
                } else {
                    console.error('Некорректные данные:', data);
                    tg.showAlert('Ошибка при получении QR-кода');
                }
            } catch (e) {
                console.error('Ошибка при обработке ответа:', e);
                tg.showAlert('Ошибка при обработке ответа от бота');
            }
        });
